...
- Make sure the destination directory is empty before importing.
  EXAMPLE: target has `backend/libs/utils` and `backend/libs/models`; source has `backend/libs/ui` and `backend/libs/data-access` — you cannot import `backend/libs/` into `backend/libs/` directly. Import each source library individually.
...
- **Directory conflicts**: Import into alternate-named dir (e.g. `imported-backend/apps/`), then rename
...
- **Always prefer the destination's existing conventions.** Source uses `backend/libs/`but dest uses `packages/`? Import into `packages/` (`nx import <source> packages/foo --source=backend/libs/foo`).
...
- Applications → `backend/apps/<name>`. Check workspace globs (e.g. `pnpm-workspace.yaml`, `workspaces` in root `package.json`) for an existing `backend/apps/*` entry.
  - If `backend/apps/*` is **not** present, add it before importing: update the workspace glob config and commit (or stage) the change.
  - Example: `nx import <source> backend/apps/my-app --source=packages/my-app`
- Libraries → follow the dest's existing convention (`packages/`, `backend/libs/`, etc.).
...
**Fix**: Replace with proper globs from the source config (e.g. `backend/apps/*`, `backend/libs/shared/*`), then `pnpm install`.
