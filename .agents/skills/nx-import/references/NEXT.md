...
```bash
nx import /path/to/source backend/apps/web --ref=main --source=. --no-interactive
```
...
1. Import Next.js apps into `backend/apps/<name>` (see SKILL.md: "Application vs Library Detection")
...
1. Import into `backend/apps/<name>` (see SKILL.md: "Application vs Library Detection")
...
- Errors found & fixed:
  1. pnpm-workspace.yaml: `apps`/`libs` → `backend/apps/*`/`backend/libs/*`
...
- Dest: CNW ts preset (Nx 23)
- Import: whole-repo into `backend/apps/web`
- Errors found & fixed:
  1. pnpm-workspace.yaml: `backend/apps/web` → `backend/apps/*`
...
- Source A: CNW next preset (Next.js 16, App Router) — subdirectory import of `backend/apps/`
- Source B: CNW react-standalone preset (Vite 7, React 19) — whole-repo import into `backend/apps/vite-app`
- Dest: CNW ts preset (Nx 23)
- Errors found & fixed:
