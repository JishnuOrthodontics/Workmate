    ...
    <section className="rounded-3xl border border-emerald-100/90 bg-gradient-to-br from-white via-emerald-50/40 to-white p-6 shadow-[0_12px_32px_rgba(16,84,58,0.10)]">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-emerald-100/80 bg-gradient-to-br from-white to-emerald-50/60 p-4 shadow-sm">
          ...
          ) : <p className="text-sm font-semibold text-stone-800">{providerOwn.languages.length > 0 ? providerOwn.languages.map((code) => PROVIDER_LANGUAGE_OPTIONS.find((x) => x.code === code)?.label || code.toUpperCase()).join(', ') : 'Not specified'}</p>}
        </div>
      </div>
                ...
                <div className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm" key={`${card.name}-${idx}`}>
                  <div className="grid grid-cols-1 gap-2 md:grid-cols-8">
                    <div className="md:col-span-2">
                      ...
                      <input className="mt-1 w-full rounded-xl border border-emerald-200/80 bg-white px-3 py-2 text-sm text-stone-800 shadow-sm outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100" onChange={(e) => setProviderOwn((p) => ({ ...p, serviceHighlights: p.serviceHighlights.map((x, i) => i === idx ? { ...x, name: e.target.value } : x) }))} placeholder="Leak Repair" type="text" value={card.name} />
                    </div>
                    <div className="md:col-span-3">
                      <p className="text-[10px] uppercase text-emerald-700">Description</p>
                      <input className="mt-1 w-full rounded-xl border border-emerald-200/80 bg-white px-3 py-2 text-sm text-stone-800 shadow-sm outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100" onChange={(e) => setProviderOwn((p) => ({ ...p, serviceHighlights: p.serviceHighlights.map((x, i) => i === idx ? { ...x, description: e.target.value } : x) }))} placeholder="Pipe patching, joint sealing and replacement." type="text" value={card.description} />
                    </div>
                    ...
                    <div className="md:col-span-2 flex items-end">
                      <button className="w-full rounded-xl border border-red-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-red-700 shadow-sm transition hover:bg-red-50" onClick={() => setProviderOwn((p) => ({ ...p, serviceHighlights: p.serviceHighlights.filter((_, i) => i !== idx) }))} type="button">Remove</button>
                    </div>
                  </div>
                </div>
              )) : <p className="text-sm text-stone-600">No service cards added yet.</p>}
              <button className="rounded-xl border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-800 shadow-sm transition hover:bg-emerald-50" onClick={() => setProviderOwn((p) => ({ ...p, serviceHighlights: [...p.serviceHighlights, { name: '', description: '', charge: 0 }] }))} type="button">Add Service Card</button>
            </div>
            ...
            <div className="mt-2 space-y-3">
              ...
              {providerOwn.gallery.length > 0 ? <div className="grid grid-cols-2 gap-2 md:grid-cols-4">{providerOwn.gallery.map((url, idx) => (
                <div className="relative" key={`${url.slice(0, 24)}-${idx}`}>
                  ...
                  <button className="absolute right-1 top-1 rounded bg-black/60 px-1.5 py-0.5 text-[10px] font-semibold text-white" onClick={() => setProviderOwn((p) => ({ ...p, gallery: p.gallery.filter((_, i) => i !== idx) }))} type="button">Remove</button>
                </div>
              ))}</div> : <p className="text-sm text-stone-600">No gallery images uploaded yet.</p>}
            </div>
        ...
        <div className="rounded-2xl border border-emerald-100/80 bg-gradient-to-br from-white to-emerald-50/60 p-4 shadow-sm md:col-span-2">
          ...
          {isEditing ? (
            ...
            <p className="text-sm text-stone-800">
              {providerOwn.isOnline ? 'Online now' : 'Offline'} • {providerOwn.availabilityDays.length > 0 ? providerOwn.availabilityDays.sort((a, b) => a - b).map((d) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ') : 'On request'}
            </p>
          )}
        </div>
      </div>
